package com.stephen.Match;

import com.stephen.BaseStats.BaseStats_Key;
import com.stephen.BaseStats.BaseStats_Service;
import com.stephen.Frame.Frame;
import com.stephen.Frame.FrameFactory.FrameFactory;
import com.stephen.Player.Player;
import com.stephen.BaseStats.StatField;
import org.slf4j.LoggerFactory;
import org.slf4j.Logger;


public class Match_Singles extends Match<Player>{
    private final BaseStats_Key matchKey;
    private static final Logger log = LoggerFactory.getLogger(Match_Singles.class);


    // --- CONSTRUCTOR ---
    public Match_Singles(Player p1, Player p2, int frameCount, FrameFactory<Player> ff) {
        super(p1, p2, frameCount, ff);
        this.matchKey = new BaseStats_Key(super.getID(), null);
        super.isPlayed = false;
        super.isBye = false;
        super.isDraw = false;
        updateCloud_Match();
    }

    public Match_Singles(Player p, int frameCount, FrameFactory<Player> frameFactory){
        super(p, frameCount, frameFactory);
        this.matchKey = new BaseStats_Key(super.getID(), null);
        super.isPlayed = false;
        super.isBye = true;
        super.isDraw = false;
        updateCloud_Match();
    }

    public Match_Singles(FrameFactory<Player> frameFactory){
        super(Player.createBye(), Player.createBye(), 0, frameFactory);
        this.matchKey = new BaseStats_Key(super.getID(), null);
        this.isPlayed = true;
        this.isBye = true;
        this.isDraw = false;
    }


    // --- MATCH OVERRIDE ---
    public void playOutMatch(){
        handleBye();
        if(!super.isBye()){
            this.playMatch();
        }
        this.recordMatch();
        this.updateCloud_Match();
        log.info("Match {} has been played", super.getID());
    }

    @Override
    public Player createByeParty() {
        return new Player();
    }

    @Override
    public void playMatch(){
        if(!isBye){
            for (int i = 0; i < this.getFrameCount(); i++) {
                Frame<Player> f = super.frameFactory.createFrame(party1, party2);
                super.frames.add(f);
                f.playOutFrame();
            }
            isPlayed = true;
            long party1Wins = frames.stream()
                    .filter(f -> f.getWinner().equals(party1))
                    .count();
            long party2Wins = frames.stream()
                    .filter(f -> f.getWinner().equals(party2))
                    .count();
            if (party1Wins > party2Wins) {
                winner = party1;
                loser = party2;
            } else if (party2Wins > party1Wins) {
                winner = party2;
                loser = party1;
            } else {
                isDraw = true;
                winner = null;
                loser = null;
            }
        }
        log.info("Played Match_Singles: {} vs {}. Result: {}", party1.getName(), party2.getName(), isDraw ? "Draw" : (getWinner().getName() + " wins"));
    }


    // --- GETTERS ---
    public int getTotalFrames(){
        return super.getFrameCount();
    }


    // --- LOGIC ---
    public void recordPlayer_Match(Match<Player> m) {
        BaseStats_Service.applyEvent(matchKey, StatField.MATCH_TOTAL, m.getParty1());
        BaseStats_Service.applyEvent(matchKey, StatField.MATCH_TOTAL, m.getParty2());
        if(this.isPlayed()){
            if (super.isDraw()) {
                BaseStats_Service.applyEvent(matchKey, StatField.MATCH_DRAW, m.getParty1());
                BaseStats_Service.applyEvent(matchKey, StatField.MATCH_DRAW, m.getParty2());
            } else if (this.getWinner().equals(super.getParty1())) {
                BaseStats_Service.applyEvent(matchKey, StatField.MATCH_WIN, m.getParty1());
                BaseStats_Service.applyEvent(matchKey, StatField.MATCH_LOSS, m.getParty2());
            } else {
                BaseStats_Service.applyEvent(matchKey, StatField.MATCH_LOSS, m.getParty1());
                BaseStats_Service.applyEvent(matchKey, StatField.MATCH_WIN, m.getParty2());
            }
        }
        log.info("Recorded player match stats for Match_Singles: {} vs {}",
                m.getParty1().getName(),
                m.getParty2().getName()
        );
    }

    public void recordMatch() {
        recordPlayer_Match(this);
        updateCloud_Match();
    }
}
